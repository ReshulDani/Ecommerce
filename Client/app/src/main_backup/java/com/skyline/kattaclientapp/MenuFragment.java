...
public class MenuFragment extends Fragment implements AsyncTaskComplete {
    ...
    public static int setMenu(List<OrderItem> orderItems, Context context) {
        int count = 0;
        int count1 = 0;
        if (orderItems.get(orderItems.size() - 1).getItemName().matches("Service Charge") || orderItems.get(orderItems.size() - 1).getItemName().matches("Discounts")) {
            //orderItems.remove(orderItems.size()-1);
            count1++;
        }
        if (orderItems.get(orderItems.size() - 2).getItemName().matches("Service Charge") || orderItems.get(orderItems.size() - 1).getItemName().matches("Discounts")) {
            //orderItems.remove(orderItems.size()-1);
            count1++;
        }
