...
public class OrderViewActivity extends BaseActivity implements AsyncTaskComplete {
    ...
    public void handleResult(JsonObject result, String action) throws JSONException {
        ...
        if (action.matches("ReadUserOrders")) {
            ...
            SimpleDateFormat dateFormat = new SimpleDateFormat("MMMM d, yyyy hh:mm a", Locale.ENGLISH);
            ...
            if (result.get("success").getAsInt() == 1) {
                ...
                for (int i = 0; i < jsonArray.size(); i++) {
                    ...
                    try {
                        orderObject.setTimestamp(dateFormat.format(dateParse.parse(jsonArray.get(i).getAsJsonObject().get("created_at").getAsString())));
